...
///
/// Groups (in display order): Difficulty (presets), Starting Options (initial
/// spawn), Balance (raw strength/speed/morale numbers), AI (decision
/// thresholds/behavior toggles), Size Behaviour (horde-size mechanics: greedy
/// growth, splitting, respawn, medium/large thresholds), Debug (dev/testing
/// toggles, unaffected by the reorg above).
/// </summary>
public sealed class ZombiePlagueSettings : AttributeGlobalSettings<ZombiePlagueSettings>
{
	...
	[SettingPropertyInteger("Number Of Starting Parties", 1, 10, Order = 0, RequireRestart = false, HintText = "How many zombie hordes spawn at random towns/villages when the initial spawn triggers.")]
	[SettingPropertyGroup("Starting Options", GroupOrder = 1)]
	...
	[SettingPropertyInteger("Starting Group Size", 1, 100, Order = 1, RequireRestart = false, HintText = "Troop count of each starting horde.")]
	[SettingPropertyGroup("Starting Options", GroupOrder = 1)]
	...
	[SettingPropertyDropdown("Starting Troop Type", Order = 2, RequireRestart = false, HintText = "Which troop the starting horde(s) are made of.")]
	[SettingPropertyGroup("Starting Options", GroupOrder = 1)]
	...
	[SettingPropertyFloatingInteger("Spawn Delay (days)", 0f, 30f, Order = 3, RequireRestart = false, HintText = "In-game days after a new campaign starts before the initial zombie horde(s) spawn.")]
	[SettingPropertyGroup("Starting Options", GroupOrder = 1)]
	...
	[SettingPropertyGroup("Balance", GroupOrder = 2)]
	...
	[SettingPropertyFloatingInteger("Hero Turn Troop Size Modifier", 0f, 1f, Order = 31, RequireRestart = false, HintText = "Extra turn chance a small horde (under Small Horde Max, see Size Behaviour) gets on top of the base chance.")]
	[SettingPropertyGroup("Balance", GroupOrder = 2)]
	...
	[SettingPropertyGroup("AI", GroupOrder = 3)]
	...
	[SettingPropertyInteger("Settlement Siege Troop Threshold", 1, 5000, Order = 52, RequireRestart = false, HintText = "Minimum troop count for a hero-led horde to besiege a town/castle at all. Meant to line up with Large Horde Threshold (Size Behaviour) - sieging is the 'large horde' behavior.")]
	[SettingPropertyGroup("AI", GroupOrder = 3)]
	...
	// --- Size Behaviour: small/medium/large horde thresholds ---

	[SettingPropertyInteger("Small Horde Max", 1, 2000, Order = 0, RequireRestart = false, HintText = "Reference horde size used by the hero-turn-chance formula (see Balance) - a horde at or under this size gets the full size bonus to its turn chance.")]
	[SettingPropertyGroup("Size Behaviour", GroupOrder = 4)]
	...
	[SettingPropertyInteger("Medium Horde Threshold", 1, 5000, Order = 1, RequireRestart = false, HintText = "Troop count (per party) at which a horde is considered 'medium' - shifts its strategic priorities toward raiding/sieging, and triggers the 'first medium horde' notification.")]
	[SettingPropertyGroup("Size Behaviour", GroupOrder = 4)]
	...
	// --- Size Behaviour: greedy growth ---

	[SettingPropertyInteger("Greedy Growth Troop Threshold", 1, 5000, Order = 10, RequireRestart = false, HintText = "Troop count at which the horde stops being picky about risk/distance and just goes after whatever is available. Meant to line up with Medium Horde Threshold above - greedy mode is the 'medium horde' behavior.")]
	[SettingPropertyGroup("Size Behaviour", GroupOrder = 4)]
	...
	[SettingPropertyFloatingInteger("Greedy Min Strength Ratio", 0.05f, 2f, Order = 11, RequireRestart = false, HintText = "Flat hunt strength-ratio floor while greedy (looser than the normal dynamic bracket).")]
	[SettingPropertyGroup("Size Behaviour", GroupOrder = 4)]
	...
	[SettingPropertyInteger("Greedy Village Raid Troop Threshold", 1, 2000, Order = 12, RequireRestart = false, HintText = "Village raid troop threshold used once the horde is greedy - usually lower than the normal one.")]
	[SettingPropertyGroup("Size Behaviour", GroupOrder = 4)]
	...
	[SettingPropertyFloatingInteger("Greedy Search Radius Multiplier", 0.5f, 5f, Order = 13, RequireRestart = false, HintText = "Hunting search radius multiplier while greedy - casts a wider net than the normal spawn radius.")]
	[SettingPropertyGroup("Size Behaviour", GroupOrder = 4)]
	...
	[SettingPropertyFloatingInteger("Greedy Hunt Distance Exponent", 1f, 5f, Order = 14, RequireRestart = false, HintText = "Exponent applied to hunt distance while greedy - higher values make proximity dominate over target size.")]
	[SettingPropertyGroup("Size Behaviour", GroupOrder = 4)]
	...
	// --- Size Behaviour: splitting ---

	[SettingPropertyInteger("Split Min Troops", 1, 10000, Order = 20, RequireRestart = false, HintText = "Below this troop count a horde never splits.")]
	[SettingPropertyGroup("Size Behaviour", GroupOrder = 4)]
	...
	[SettingPropertyInteger("Split Max Troops", 1, 10000, Order = 21, RequireRestart = false, HintText = "At or above this troop count a horde is guaranteed to split every day.")]
	[SettingPropertyGroup("Size Behaviour", GroupOrder = 4)]
	...
	[SettingPropertyFloatingInteger("Split Fraction", 0.05f, 0.9f, Order = 22, RequireRestart = false, HintText = "Fraction of the horde's troops that leave with the newly split-off party.")]
	[SettingPropertyGroup("Size Behaviour", GroupOrder = 4)]
	...
	[SettingPropertyFloatingInteger("Split Away Distance Factor", 0.1f, 3f, Order = 23, RequireRestart = false, HintText = "How far the split-off party gets redirected away from its parent, relative to the spawn radius.")]
	[SettingPropertyGroup("Size Behaviour", GroupOrder = 4)]
	...
	[SettingPropertyFloatingInteger("Split Spawn Max Distance Factor", 0.01f, 1f, Order = 24, RequireRestart = false, HintText = "Upper bound for where the split-off party initially spawns relative to its parent.")]
	[SettingPropertyGroup("Size Behaviour", GroupOrder = 4)]
	...
	[SettingPropertyFloatingInteger("Split Spawn Min Distance Factor", 0.01f, 1f, Order = 25, RequireRestart = false, HintText = "Lower bound for where the split-off party initially spawns relative to its parent.")]
	[SettingPropertyGroup("Size Behaviour", GroupOrder = 4)]
	...
	// --- Size Behaviour: respawn after total defeat ---

	[SettingPropertyFloatingInteger("Respawn Max Distance Factor", 0.01f, 3f, Order = 30, RequireRestart = false, HintText = "Upper bound for how far a replacement horde spawns from where the old one died.")]
	[SettingPropertyGroup("Size Behaviour", GroupOrder = 4)]
	...
	[SettingPropertyFloatingInteger("Respawn Min Distance Factor", 0.01f, 3f, Order = 31, RequireRestart = false, HintText = "Lower bound for how far a replacement horde spawns from where the old one died.")]
	[SettingPropertyGroup("Size Behaviour", GroupOrder = 4)]
